...
type CustomGroceryItem = {
  name: string;
  sections: Record<StoreName, string>;
};
...
export default function Home() {
  ...
  const [customGroceries, setCustomGroceries] = useState<CustomGroceryItem[]>([]);
  const [bankItemName, setBankItemName] = useState("");
  const [bankKrogerSection, setBankKrogerSection] = useState("Custom");
  const [bankWalmartSection, setBankWalmartSection] = useState("Custom");
  const [bankAldiSection, setBankAldiSection] = useState("Custom");

  const allGroceries = useMemo<GroceryItem[]>(
    () => [...groceries, ...customGroceries],
    [customGroceries]
  );
  ...
  function normalizeSelectedItems(items: SelectedItem[], store: StoreName): SelectedItem[] {
    return items
      .map((item) => {
        const match = allGroceries.find((g) => g.name === item.name);
        if (!match) {
          return {
            ...item,
  ...
  useEffect(() => {
    ...
    const savedCustomGroceries = safeParse<CustomGroceryItem[]>(
      localStorage.getItem("groceryCustomBank"),
      []
    );
    setCustomGroceries(savedCustomGroceries);
  ...
  useEffect(() => {
    localStorage.setItem("groceryCustomBank", JSON.stringify(customGroceries));
  }, [customGroceries]);

  useEffect(() => {
    setSelectedItems((prev) => normalizeSelectedItems(prev, selectedStore));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedStore, customGroceries]);
  ...
  function addSelectedGroceryFromDropdown() {
    ...
    const groceryItem = allGroceries.find((item) => item.name === selectedGroceryName);
  ...
  function addQuickDropdownItem() {
    ...
    const groceryItem = allGroceries.find((item) => item.name === selectedQuickAddName);
    if (!groceryItem) return;
    addItem(groceryItem);
  ...
  function addItemToGroceryBank() {
    const trimmedName = bankItemName.trim();
    if (!trimmedName) return;

    const alreadyExists = allGroceries.some(
      (item) => item.name.toLowerCase() === trimmedName.toLowerCase()
    );

    if (alreadyExists) {
      alert("That grocery item already exists in the grocery bank.");
      return;
    }

    const newBankItem: CustomGroceryItem = {
      name: trimmedName,
      sections: {
        "Kroger Hernando": bankKrogerSection.trim() || "Custom",
        "Walmart Hernando": bankWalmartSection.trim() || "Custom",
        ALDI: bankAldiSection.trim() || "Custom",
      },
    };

    setCustomGroceries((prev) => [...prev, newBankItem]);

    setBankItemName("");
    setBankKrogerSection("Custom");
    setBankWalmartSection("Custom");
    setBankAldiSection("Custom");
  }

  function removeCustomBankItem(itemName: string) {
    setCustomGroceries((prev) => prev.filter((item) => item.name !== itemName));
  ...
  }, [allGroceries, searchText, selectedSection, selectedStore]);

  const favoriteGroceries = useMemo(
    () => allGroceries.filter((item) => favoriteItems.includes(item.name)),
    [allGroceries, favoriteItems]
  );
  ...
  const suggestedItems = Array.from(
    ...
    .map((name) => allGroceries.find((g) => g.name === name))
    .filter(Boolean) as GroceryItem[];

  const grocerySections = Array.from(new Set(allGroceries.map((item) => getItemSection(item))));
  ...
  const sections = useMemo(
    ...
    [grocerySections, customSections]
  );
  ...
  const quickAddOptions = quickAddItems
    .map((itemName) => allGroceries.find((item) => item.name === itemName))
    .filter(Boolean) as GroceryItem[];
  ...
  return (
    ...
    >
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        {appMode === "build" && (
          <details style={detailsStyle}>
            <summary style={summaryStyle}>Add Custom Item To Grocery Bank</summary>
            <div
              style={{
                padding: "12px",
                display: "grid",
            ...
            >
              ...
              >
                <input
                  type="text"
                  placeholder="Kroger section"
                  value={bankKrogerSection}
                  onChange={(e) => setBankKrogerSection(e.target.value)}
                  style={inputStyle}
                />

                <input
                  type="text"
                  placeholder="Walmart section"
                  value={bankWalmartSection}
                  onChange={(e) => setBankWalmartSection(e.target.value)}
                  style={inputStyle}
                />

                <input
                  type="text"
                  placeholder="ALDI section"
                  value={bankAldiSection}
                  onChange={(e) => setBankAldiSection(e.target.value)}
                  style={inputStyle}
                />
              </div>
                    ...
                    {customGroceries.map((item) => (
                      ...
                      >
                        <div>
                          ...
                          <div style={{ fontSize: "11px", color: theme.mutedText }}>
                            Kroger: {item.sections["Kroger Hernando"]} • Walmart: {item.sections["Walmart Hernando"]} • ALDI: {item.sections["ALDI"]}
                          </div>
                        </div>

                        <button
                          onClick={() => removeCustomBankItem(item.name)}
                          style={{
                            ...miniButtonStyle,
                            backgroundColor: theme.danger,
                            color: theme.white,
